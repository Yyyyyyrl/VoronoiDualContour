    ...
    std::vector<std::vector<int>> &facet_midpoint_indices)
{
    for (size_t i = 0; i < vc.facet_indices.size(); ++i)
    {
        ...
        // Compute matches using the routine and connect midpoints accordingly
        BIPOLAR_MATCH_METHOD method = BIPOLAR_MATCH_METHOD::SEP_NEG;  // Can change, placeholder
        auto pairs = match_facet_bipolar_edges(voronoiDiagram, facet_index, isovalue, method);
        for (const auto& p : pairs) {
            if (p.first < current_facet_midpoints.size() && p.second < current_facet_midpoints.size()) {
                int idx1 = current_facet_midpoints[p.first];
                int idx2 = current_facet_midpoints[p.second];
                midpoints[idx1].connected_to.push_back(idx2);
                midpoints[idx2].connected_to.push_back(idx1);
            }
        }

        //auto types = get_bipolar_match_types(voronoiDiagram, facet_index, isovalue);
...
//
void order_facet_vertices(std::vector<int> &indices,
                          const Point &p0,
                          const Point &p1,
...
std::vector<std::pair<int, int>> match_facet_bipolar_edges(
    ...
    const VoronoiCellFacet& facet = voronoiDiagram.facets[facet_index];
    const auto& vert_indices = facet.vertices_indices;
    int num = vert_indices.size();

    // Collect bipolar edges and their types (true if pos->neg)
    std::vector<int> bipolar_edge_indices;  // Original edge indices (0 to num-1) that are bipolar
    std::vector<bool> is_pos_to_neg;        // Type for each bipolar edge
    for (int i = 0; i < num; ++i) {
        int vi0 = vert_indices[i];
        int vi1 = vert_indices[(i + 1) % num];
        float val0 = voronoiDiagram.vertices[vi0].value;
        float val1 = voronoiDiagram.vertices[vi1].value;
        if (is_bipolar(val0, val1, isovalue)) {
            bipolar_edge_indices.push_back(i);
            is_pos_to_neg.push_back((val0 >= isovalue) && (val1 < isovalue));
        }
    }
    int num_bipolar = bipolar_edge_indices.size();

    if (num_bipolar == 0 || num_bipolar % 2 != 0) {
        // No midpoints or odd number (should not happen, but skip)
        return matches;
    }

    if (match_method == BIPOLAR_MATCH_METHOD::UNCONSTRAINED_MATCH) {
        // Placeholder arbitrary pairing: 0-1, 2-3, etc. (local indices)
        for (int k = 0; k < num_bipolar; k += 2) {
            matches.emplace_back(k, k + 1);
        }
    } else if (match_method == BIPOLAR_MATCH_METHOD::SEP_POS || match_method == BIPOLAR_MATCH_METHOD::SEP_NEG) {
        bool use_pos_neg_start = (match_method == BIPOLAR_MATCH_METHOD::SEP_NEG);  // true for SEP_NEG (start with pos->neg)

        // Collect start positions (local bipolar indices where type matches start)
        std::vector<int> starts;
        for (int k = 0; k < num_bipolar; ++k) {
            if (is_pos_to_neg[k] == use_pos_neg_start) {
                starts.push_back(k);
            }
        }

        // Pair each start with the next (which should be opposite type)
        for (int s : starts) {
            int next = (s + 1) % num_bipolar;
            // Validate opposite type (assume alternating; add check)
            if (is_pos_to_neg[next] == use_pos_neg_start) {
                std::cerr << "[WARNING] Consecutive bipolar edges have same transition type on facet " << facet_index << std::endl;
                continue;
            }
            matches.emplace_back(s, next);
        }
    } else {
        std::cerr << "[ERROR] Undefined match method on facet " << facet_index << std::endl;
    }
