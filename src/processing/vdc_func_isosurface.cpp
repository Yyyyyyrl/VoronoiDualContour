...
// Single-isovertex functions moved to vdc_func_isosurface_single.cpp
    ...
    const std::vector<int> *vertex_mapping = nullptr)
{
    ...
    const auto [idx_v1, idx_v2] = std::minmax(edge.vertex1, edge.vertex2);
...
// compute_isosurface_vertices_single moved to vdc_func_isosurface_single.cpp
...
static void compute_cycle_centroids_and_isovertices(
    ...
    bool disable_cycle_clipping = false)
{
    if (ISO_DBG_ENABLED && iso_dbg_cell_ok(vc.cellIndex))
    {
 ...
 */
void compute_isosurface_vertices_multi(VoronoiDiagram &voronoiDiagram, float isovalue, IsoSurface &iso_surface, const UnifiedGrid *grid, const std::vector<int> *vertex_mapping, const std::vector<Point> *accurateIsoCrossings, bool disable_cycle_clipping)
{
    ISO_DBG_LOAD_ENV();
    ...
    for (auto &vc : voronoiDiagram.cells)
    {
        ...
        compute_cycle_centroids_and_isovertices(
            vc, voronoiDiagram, midpoints, cycles, iso_surface, cube_side_length, accurate_crossing, disable_cycle_clipping);
    }
}
...
void construct_iso_surface(Delaunay &dt, VoronoiDiagram &vd, VdcParam &vdc_param, IsoSurface &iso_surface, UnifiedGrid &grid, std::vector<Point> &activeCubeCenters, std::vector<Point> &activeCubeAccurateIsoCrossingPoints, CGAL::Epick::Iso_cuboid_3 &bbox, const std::vector<int> *vertex_mapping, int *out_interior_flips, int *out_boundary_flips, int *out_total_flips, std::size_t *out_clipped_count, double *out_max_clip_distance)
{
    ...
    // Handle single_isov mode separately (simple path without conflict resolution)
    if (!vdc_param.multi_isov)
    {
        ...
        timer.stopTimer("Compute isosurface vertices", "7. Isosurface Construction");
        ...
        timer.stopTimer("Compute dual triangles", "7. Isosurface Construction");
        return;
    }

    // Multi-isovertex mode: complex iterative conflict resolution
    {
        ...
        for (int attempt = 0; attempt < maxResolve && !dualBuilt; ++attempt)
        {
            ...
            if (matchesDirty)
            {
                if (recomputeAllMatches)
                {
                    ...
                    timer.stopTimer("Compute bipolar matches", "7. Isosurface Construction");
                    ...
                    timer.stopTimer("Build iso segments", "7. Isosurface Construction");
                }
                else
                {
            ...
            timer.startTimer("Compute isosurface vertices", "7. Isosurface Construction");
            compute_isosurface_vertices_multi(vd, vdc_param.isovalue, iso_surface, &grid, vertex_mapping, &activeCubeAccurateIsoCrossingPoints, vdc_param.noclip);
            timer.stopTimer("Compute isosurface vertices", "7. Isosurface Construction");
            ...
            if (vdc_param.mod_cyc)
            {
                ...
                timer.stopTimer("Modify cycles", "7. Isosurface Construction");
    ...
    // Debug output for multi-isovertex mode
    if (std::getenv("MODCYC_DEBUG"))
    {
        dump_iso_vertex_cycles(vd, iso_surface, "modcyc_cycles.csv");
        dump_triangle_sources(iso_surface, "modcyc_triangles.csv");
    }

    // Fallback: build dual triangles if not already done
    if (!dualBuilt)
    {
        bindingConflict = compute_dual_triangles_multi(vd, bbox, grid, vdc_param.isovalue, iso_surface, vertex_mapping);
    }
